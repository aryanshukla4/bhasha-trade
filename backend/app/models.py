...
class Order(Base):
    __tablename__='orders'; id: Mapped[str]=mapped_column(String,primary_key=True); listing_id: Mapped[str]=mapped_column(ForeignKey('produce_listings.id')); buyer_id: Mapped[str]=mapped_column(ForeignKey('users.id')); farmer_id: Mapped[str]=mapped_column(ForeignKey('users.id')); agreed_price: Mapped[float]=mapped_column(Float); status: Mapped[str]=mapped_column(String,default='pending'); created_at: Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utcnow)
